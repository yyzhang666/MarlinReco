...
IsolatedLeptonFinderProcessor::IsolatedLeptonFinderProcessor()
  ...
  registerInputCollection(LCIO::RECONSTRUCTEDPARTICLE, "InputCollection", "Input collection of ReconstructedParticles",
  ...
  registerProcessorParameter("CosConeAngle", "Cosine of the half-angle of the cone used in isolation criteria",
                             _cosConeAngle, float(0.98));
  ...
  registerProcessorParameter("ElectronMinEcalToHcalFraction",
                             ...
                             _electronMinEcalToHcalFraction, float(0.9));
  ...
  registerProcessorParameter("ElectronMaxEcalToHcalFraction",
                             ...
                             _electronMaxEcalToHcalFraction, float(1.0));

  registerProcessorParameter("MuonMinEnergyDepositByMomentum", "Muon ID: Minimum energy deposit divided by momentum",
                             _muonMinEnergyDepositByMomentum, float(0.0));

  registerProcessorParameter("MuonMaxEnergyDepositByMomentum", "Muon ID: Maximum energy deposit divided by momentum",
                             _muonMaxEnergyDepositByMomentum, float(0.3));
  ...
  registerProcessorParameter("MuonMinEcalToHcalFraction",
                             ...
                             _muonMinEcalToHcalFraction, float(0.0));
  ...
  registerProcessorParameter("ImpactParameterMaxD0", "Maximum d0 impact parameter", _maxD0, float(1e20));

  registerProcessorParameter("ImpactParameterMinZ0", "Minimum z0 impact parameter", _minZ0, float(0.0));

  registerProcessorParameter("ImpactParameterMaxZ0", "Maximum z0 impact parameter", _maxZ0, float(1e20));

  registerProcessorParameter("ImpactParameterMin3D", "Minimum impact parameter in 3D", _minR0, float(0.0));

  registerProcessorParameter("ImpactParameterMax3D", "Maximum impact parameter in 3D", _maxR0, float(0.01));
  ...
  registerProcessorParameter("UseImpactParameterSignificance",
                             ...
                             _useImpactParameterSignificance, bool(false));

  registerProcessorParameter("ImpactParameterMinD0Significance", "Minimum d0 impact parameter significance", _minD0Sig,
                             float(0.0));

  registerProcessorParameter("ImpactParameterMaxD0Significance", "Maximum d0 impact parameter significance", _maxD0Sig,
                             float(1e20));

  registerProcessorParameter("ImpactParameterMinZ0Significance", "Minimum z0 impact parameter significance", _minZ0Sig,
                             float(0.0));
  ...
  registerProcessorParameter("IsolationMinimumTrackEnergy", "Minimum track energy for isolation requirement",
                             _isoMinTrackEnergy, float(15));

  registerProcessorParameter("IsolationMaximumTrackEnergy", "Maximum track energy for isolation requirement",
                             _isoMaxTrackEnergy, float(1e20));

  registerProcessorParameter("IsolationMinimumConeEnergy", "Minimum cone energy for isolation requirement",
                             _isoMinConeEnergy, float(0));

  registerProcessorParameter("IsolationMaximumConeEnergy", "Maximum cone energy for isolation requirement",
                             _isoMaxConeEnergy, float(1e20));

  registerProcessorParameter("UsePolynomialIsolation", "Use polynomial cuts on track and cone energy",
                             _usePolynomialIsolation, bool(true));
  ...
  registerProcessorParameter("IsolationPolynomialCutC",
                             ...
                             _isoPolynomialC, float(-300));

  registerProcessorParameter("UseJetIsolation", "Use jet-based isolation", _useJetIsolation, bool(false));
  ...
  registerProcessorParameter("DressPhotonConeAngle",
                             ...
                             _dressPhotonConeAngle, float(1));

  registerProcessorParameter("MergeLeptonConeAngle", "Half-angle (in degrees) of the cone used for lepton merging",
                             _mergeLeptonConeAngle, float(2));

  registerProcessorParameter("UsePandoraIDs", "Use Pandora particle IDs for algorithm", _usePandoraIDs, bool(false));
}
...
void IsolatedLeptonFinderProcessor::processEvent(LCEvent* evt) {
  ...
  // order by energy
  std::sort(goodLeptonIndices.begin(), goodLeptonIndices.end(),
            [this](const int i, const int j) { return isMoreEnergetic(i, j); });
...
void IsolatedLeptonFinderProcessor::dressLepton(ReconstructedParticleImpl* pfo, int PFO_idx) {
  ...
  for (int i = 0; i < npfo; i++) {
    ...
    if (!_mergeCloseElectrons && isElectron)
      continue;
    ...
    if ((isPhoton && theta <= _dressPhotonConeAngle) || (isElectron && theta <= _mergeLeptonConeAngle)) {
      if (std::find(_dressedPFOs.begin(), _dressedPFOs.end(), i) != _dressedPFOs.end()) {
        if (isPhoton) {
          streamlog_out(DEBUG) << "WARNING: photon " << i << " with theta " << theta << " and type " << pfo->getType()
...
bool IsolatedLeptonFinderProcessor::IsElectron(ReconstructedParticle* pfo) {
  ...
  if (calByP >= _electronMinEnergyDepositByMomentum && calByP <= _electronMaxEnergyDepositByMomentum &&
      ecalFrac >= _electronMinEcalToHcalFraction && ecalFrac <= _electronMaxEcalToHcalFraction)
    return true;
...
bool IsolatedLeptonFinderProcessor::IsMuon(ReconstructedParticle* pfo) {
  ...
  if (calByP >= _muonMinEnergyDepositByMomentum && calByP <= _muonMaxEnergyDepositByMomentum &&
      ecalFrac >= _muonMinEcalToHcalFraction && ecalFrac <= _muonMaxEcalToHcalFraction)
    return true;
...
bool IsolatedLeptonFinderProcessor::IsIsolatedRectangular(ReconstructedParticle* pfo) {
  ...
  if (E < _isoMinTrackEnergy)
    return false;
  if (E > _isoMaxTrackEnergy)
    return false;
  if (coneE < _isoMinConeEnergy)
    return false;
  if (coneE > _isoMaxConeEnergy)
    return false;
